...
async function editarImagenGaleria(imagenGaleria, nueva_imagenGaleria){
    ...
    const imgEditada = await imagenGaleriaModel.findOne({where: imagenGaleria}).then(async (result)=>{
        if(result){
            status = 200;
            await imagenGaleriaModel.update(nueva_imagenGaleria, {where: imagenGaleria});
            return {msg: 'imagen editada'}
        }else{
            status = 404;
            return {msg: "imagen no encontrada"}
        }
