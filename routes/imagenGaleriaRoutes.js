...
router.put('/:id', editarImagenGaleria)
...
async function editarImagenGaleria(req, res, next){
    try{
        const result = await imagenGaleriaService.editarImagenGaleria(req.params, req.body);
        //res.status(result.status).json(result.imagenModificada)
    }catch(error){
        next(error);
    }
}
